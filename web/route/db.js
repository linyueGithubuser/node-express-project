...
var userCenter=function(req,resp){
    ...
    let sql="SELECT *,(SELECT COUNT(*) FROM giftcard WHERE user_id=? GROUP BY user_id) giftNum, (SELECT COUNT(*) FROM voucher WHERE user_id=? GROUP BY user_id) vouNum FROM userinfo WHERE nickname=?";
    util.myConnect(sql,[userId,userId,dname],function(err,data){
...
var personalInformation=function(req,resp){
    ...
    util.myConnect(sql,dname,function(err,data){
        console.log(data);
...
var addess=function(req,resp){
    let dname= req.session.nickname;
    let sql="SELECT * FROM userinfo,address WHERE userinfo.nickname=? AND userinfo.user_id=address.user_id";
    util.myConnect(sql,dname,function(err,data){
        if(data!=""){
...
var coupons=function(req,resp){
    let dname= req.session.nickname;
    let sql="SELECT * FROM userinfo,voucher WHERE userinfo.nickname=? AND userinfo.user_id=voucher.user_id  ";
    util.myConnect(sql,dname,function(err,data){
        console.log(data=="");
        resp.render("coupons",{
            mydata:data,
...
var giftcard=function(req,resp){
    let dname= req.session.nickname;
    let sql="SELECT * FROM userinfo,giftcard WHERE userinfo.nickname=? AND userinfo.user_id=giftcard.user_id  ";
    util.myConnect(sql,dname,function(err,data){
        console.log(data=="");
...
var CreamTea=function(req,resp){
    let sql="SELECT * FROM product WHERE pro_img!=''AND ptype_id=14";
    util.myConnect(sql,[],function(err,data){
        var mydata=data;
        let sql="SELECT * FROM product WHERE pro_img!=''AND ptype_id=13";
        util.myConnect(sql,[],function(err,data){
            var smallcake=data;
            let sql="SELECT * FROM product WHERE pro_img!=''AND ptype_id=12";
            util.myConnect(sql,[],function(err,data){
                var normal=data;
                let sql="SELECT * FROM product WHERE pro_img!=''AND ptype_id=15";
                util.myConnect(sql,[],function(err,data){
                    var coffee=data;
...
var teaCheck=function(req,resp){
    ...
    if(teaValue=="全部分类"||teaValue=="全部口味"){
        sql="SELECT * FROM product WHERE pro_img!=''AND ptype_id=14";
    }else{
        if(teaValue=="冰淇淋"){
            teaValue="雪糕"
        }
...
var uploadImg=function(req,resp){
    ...
    util.myConnect(sql,[name,userId],function(err,data){
        let sql="SELECT titleImg FROM userinfo WHERE user_id=1";
        util.myConnect(sql,[name,userId],function(err,data){
            req.session.titleImg=data[0].titleImg;
            resp.send(name);
        })
...
var showAll=function(req,resp){
    let sql="select * from product where pro_img!=''and ptype_id!='' and pro_descript=1";
    util.myConnect(sql,[],function(err,data){
        //console.log(data);
        resp.render("cake",{
            cake:data,
...
var cakeCheck=function(req,resp){
    ...
    if(cakeValue=="全部分类"||cakeValue=="全部口味"){
        sql="select * from product where pro_img!=''and ptype_id!='' and pro_descript=1";
    }else{
...
var dingzhitijiao=function(req,resp){
    ...
    let customizedName=req.body.zhucailiao+req.body.fuchailiao;//通过用户点击获取定制蛋糕名称
    let userId=req.session.userId;//用户Id
    let sql="SELECT * FROM product WHERE pro_img=?";//查询商品Id和价格
...
var convertPro=function(req,resp){
    ...
    let param=[];
    let sql="SELECT convert_total FROM t_convert WHERE convert_state=0";
    util.myConnect(sql,[],function(err,data){
        var allTotoal=0;
        ...
        if(userIntegral >= newallTotoal){
            ...
            let sql="SELECT * FROM t_convert WHERE pro_id=(SELECT pro_id FROM product WHERE pro_name=?) AND convert_state=0";
            util.myConnect(sql,[convertN],function(err,data){
                ...
                if(data!=""){
                    ...
                    let sql="SELECT conver_num FROM t_convert WHERE pro_id=?";
                    util.myConnect(sql,conProId,function(err,data){
...
var indent=function(req,resp){
    ...
    if(userId==undefined){
        ...
        util.myConnect(sql,userId,function(err,data){
            ...
            var buydata=data;
            //查询兑换编与产品表
            let sql="SELECT * FROM t_convert c  JOIN product p ON c.pro_id=p.pro_id JOIN userinfo u ON u.user_id=c.user_id JOIN integral i ON i.integral_id=p.integral_id WHERE c.user_id=1 AND c.convert_state=0";
            util.myConnect(sql,userId,function(err,data){
...
var birthdayshow=function(req,resp){
    let sql=" SELECT* FROM product WHERE pro_img LIKE  '%birthday%'";
    util.myConnect(sql,[],function(err,data){
...
var brhgoods=function(req,resp){
    ...
    //console.log(name4[1]);
    let name5="%"+name4[1]+"%";
    let sql=" SELECT* FROM product WHERE pro_img LIKE  '%birthday%' AND pro_bimg LIKE ? ";
    util.myConnect(sql,name5,function(err,data){
        var ourdata=data;
...
var recommend=function(req,resp){
    let sql="SELECT * FROM product WHERE pro_img LIKE '%lycake%' ";
    util.myConnect(sql,[],function(err,data){
...
var brhgoodss=function(req,resp){
    ...
    //console.log(name4[1]);
    let name5="%"+name4[1]+"%";
    let sql="SELECT* FROM product WHERE pro_img LIKE  '%lycake%' AND pro_bimg LIKE ? ";
    util.myConnect(sql,name5,function(err,data){
        var ourdata=data;
        req.session.goods1=data[0].pro_descript_img_four;
        req.session.goods2=data[0].pro_descript_img_five;
...
var shopping=function(req,resp){
    ...
    let sql="SELECT * FROM product WHERE pro_img LIKE '%bir%' AND pro_bimg LIKE ? ";
    util.myConnect(sql,name5,function(err,data){
...
var tjshopping=function(req,resp){
    ...
    let order_state=0;
    let  userId=req.session.userId;
    let sql="SELECT * FROM product WHERE pro_img LIKE '%lycake%' AND pro_orimg=?";
                ...
                util.myConnect(sql5,userId,function(err,data){
                    ...
                    util.myConnect(sql8,userId,function(err,data){
                        ...
                        util.myConnect(sql6,userId,function(err,data){
                            let data6=data;
                            let sql7="SELECT * FROM t_convert dh  LEFT JOIN product pd2 ON dh.pro_id=pd2.pro_id LEFT JOIN integral jf1 ON jf1.integral_id=pd2.integral_id WHERE user_id=? AND dh.convert_state=0";
                            util.myConnect(sql7,userId,function(err,data){
                                let data7=data;
                                let sql9="SELECT SUM(integral_score) AS totalgb FROM t_convert dh  LEFT JOIN product pd2 ON dh.pro_id=pd2.pro_id LEFT JOIN integral jf1 ON jf1.integral_id=pd2.integral_id WHERE user_id=? AND dh.convert_state=0;";
...
var h_jiesuan=function(req,resp){
    ...
    //向最终订单表添加数据
    let sql="INSERT INTO finalorder (Indistribution,final_orderid,senddate,sendtime,payway,message,user_id,totalmoney,phone,username,userdress,youhuimoney,finalorder_status)VALUES (?,NULL,?,?,?,?,?,?,?,?,?,?,2);";
